...
export default function AcougScreen({ navigation }: RootTabScreenProps<'AcougScreen'>) {
  ...
  //read
  useEffect(() => {
    onValue(ref(db, 'Market List/Acougue'), (snapshot) => {
      setItems([])
  ...
  //write
  const onPress = () => {
    const newPostKey = push(child(ref(db), 'Market List/Acougue')).key
    set(ref(db, `Market List/Acougue/${newPostKey}`), {
      item: List,
      quantidade: Quant,
      checked: false,
      uuid: newPostKey,
  ...
  const changeCheck = (id: string, checked: boolean) => {
    update(ref(db, 'Market List/Acougue/' + id), {
      checked: checked,
      uuid: id,
    })
    setList('')
  ...
  const remover = (id: string) => {
    remove(ref(db, 'Market List/Acougue/' + id))
  }
  /*Funções antigas
const onPress= () => {
  set(newItemRef, {
...
*/
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Lista Açougue</Text>
