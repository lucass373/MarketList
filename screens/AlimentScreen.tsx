...
import HortScreen from './HortScreen'
...
export default function AlimentScreen(/*{ colorScheme } : {colorScheme : ColorSchemeName}*/) {
...
function AlimentScreen1({ navigation }: RootTabScreenProps<'AlimentScreen1'>) {
  ...
  //read
  useEffect(() => {
    onValue(ref(db, 'Market List/alimento'), (snapshot) => {
      setItems([])
  ...
  const onPress = () => {
    ...
    set(ref(db, `Market List/alimento/${newPostKey}`), {
      item: List,
      quantidade: Quant,
      checked: false,
      uuid: newPostKey,
  ...
  const changeCheck = (id: string, checked: boolean) => {
    update(ref(db, 'Market List/alimento/' + id), {
      checked: checked,
      uuid: id,
    })
    setList('')
  ...
  const remover = (id: string) => {
    remove(ref(db, 'Market List/alimento/' + id))
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FAB style={styles.fab} color={'#AFD3E2'} icon="pencil" onPress={onPress} />
